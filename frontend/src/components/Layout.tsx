...
export default function Layout({ children, accounts: propAccounts = [], onSync, syncing }: LayoutProps) {
  ...
  const Nav = () => (
    <nav className="space-y-0.5">
      ...
      <NavLink to="/projects" className={({ isActive }) => `${navItem} ${isActive ? navActive : navInactive}`}
        onClick={() => setMobileMenuOpen(false)}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      <NavLink to="/configuration" className={({ isActive }) => `${navItem} ${isActive ? navActive : navInactive}`}
        onClick={() => setMobileMenuOpen(false)}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
